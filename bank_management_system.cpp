...
public:
    ...
    static bool isAccountNumberExists(int accountNumber);
    static bool isAccountNameExists(const char *accountName);
};

void Bank_Account::displayDateTime() const
{
    time_t now = time(0);
    char *dt = ctime(&now);
    cout << "\n\tDate and Time: " << dt;
...
void Bank_Account::create_Bank_Account()
{
    ...
    do
    {
        cout << "\n\tPlease Enter The Starting Total-Money (minimum 2000): ";
        cin >> Money_Deposit;

        if (Money_Deposit < 2000)
        {
            cout << "Minimum balance should be 2000. Enter an amount greater than 2000." << endl;
        }

    } while (Money_Deposit < 2000);
...
{
    if (Money_Deposit - x < 2000)
    {
        cout << "Withdrawal failed. Minimum balance should be maintained." << endl;
    }
    else
    {
        Money_Deposit -= x;
...
void Bank_Account::writeToTextFile() const
{
    ofstream textFile("Bank_Account_Data.txt", ios::app);
    ...
    textFile << "Bank_Account No.: " << acno << "\n";
    textFile << "Bank_Account Holder Name: " << name << "\n";
    textFile << "Type of Bank_Account: " << type << "\n";
    textFile << "Balance Total-Money: " << Money_Deposit << "\n";
    textFile << "Date and Time: ";
...
bool Bank_Account::isAccountNumberExists(int accountNumber)
{
    Bank_Account ac;
...
bool Bank_Account::isAccountNameExists(const char *accountName)
{
    Bank_Account ac;
    ifstream inFile("Bank_Account.dat", ios::binary);
    ...
    while (inFile.read(reinterpret_cast<char *>(&ac), sizeof(Bank_Account)))
    {
        if (strcmp(ac.name, accountName) == 0)
        {
            inFile.close();
            return true;
        }
...
void write_Bank_Account()
{
    ...
    if (Bank_Account::isAccountNumberExists(accountNumber))
    {
        cout << "\n\tAccount with the same number already exists. Please choose a different account number." << endl;
        return;
    }
    ...
    if (Bank_Account::isAccountNameExists(accountName))
    {
        cout << "\n\tAccount with the same name already exists. Please choose a different name." << endl;
        return;
    }
...
void Updation_Bank_Account(int n)
{
    bool found = false;
    Bank_Account ac;
    fstream File("Bank_Account.dat", ios::binary | ios::in | ios::out);
